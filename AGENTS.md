...
- Shared dataset SQL filters in `deadtrees_prepackaged/postgres/filters.py` are intended to be applied against `v2_full_dataset_view` and additionally enforce `is_audited = true` plus a matching `dataset_audit.final_assessment = 'no_issues'` row by default so exports can use dataset-level audit state without depending on polygon-candidate audit fields.
...
- Current metadata columns:
  ...
  - `citation_doi`
  - `freidata_doi`
  - `phenology_probability_at_acquisition_0_100`
  - `bbox`
  - `biome_name`
